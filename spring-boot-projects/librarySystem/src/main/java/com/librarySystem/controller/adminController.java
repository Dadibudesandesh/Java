...
//    }
    ...
    @GetMapping("/admin/borrowBook/{bookId}/{studentId}")
    public String borrowBookPage(HttpSession session,@PathVariable int bookId,@PathVariable int studentId,Model model) {
        Object role = session.getAttribute("role");
        if (role == null || !role.equals("admin")) {
            return "redirect:/login";
        }
    ...
    public String borrowBookPage(HttpSession session,Model model) {
        Object role = session.getAttribute("role");
        if (role == null || !role.equals("admin")) {
            return "redirect:/login";
        }
...
//         query parameters
//        return "redirect:/admin/borrowBook?bookId=" + bookId + "&studentId=" + studentId;

        // Path Variables
        return "redirect:/admin/borrowBook/" + bookId + "/" + studentId;
    }
